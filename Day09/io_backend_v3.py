...
class Page:
  ...
  def size(self, count_empty=False):
    return len([e for e in self._data if e or count_empty])
...
class Buffer:
  ...
  def buffer_is_full(self):
    return len([x for x in self._buffer if x is None]) == 0
  ...
  def get_file_size(self, file_id, count_empty=False):
    return len([p for p in self._disk[file_id] if p or count_empty])
  ...
  def update_buffer_queue_indicator(self, remove_idx=None, add_idx=None):
    ...
    if remove_idx is not None:
      self._buffer_order = [i for i in self._buffer_order if i != remove_idx]
  ...
  def get_file(self, fid):
    ...
    if fid >= len(self._disk):
      ...
      return list(range(len(self._disk[fid])))
  ...
  def display_set_mark(self, reset_io=True):
    ...
    if reset_io:
      for k in list(self._io_count.keys()):
        self._io_count[k] = 0
...
def new_rand_file(b, r, l, sorted=False):
  vals = random.sample(list(range(r)), l)
  if sorted:
    vals.sort()
  fid = b.new_file()
  fw = FileWriter(b, fid)
